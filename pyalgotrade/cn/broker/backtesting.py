...
from pyalgotrade.cn.broker import fillstrategy
...
class Broker(broker.Broker):
    ...
    def getBuysIntraday(self, instrument):
        return self.__buysIntraday.get(instrument, 0)

    def getSalableShares(self, instrument):
        return self.getShares(instrument) - self.getBuysIntraday(instrument)
    ...
    def commitOrderExecution(self, order, dateTime, fillInfo):
        ...
        if resultingCash >= 0 or self.__allowNegativeCash:
            ...
            if order.isBuy():
                updatedBuysIntraday = order.getInstrumentTraits().roundQuantity(
                    self.getBuysIntraday(order.getInstrument()) + sharesDelta
                )
                self.__buysIntraday[order.getInstrument()] = updatedBuysIntraday
                self.__lastBuydate = dateTime.date()
    ...
    def onBars(self, dateTime, bars):
        # update the buys before when a new trading day coming
        if self.__lastBuydate == None or self.__lastBuydate < dateTime.date():
            self.__buysIntraday = {}
